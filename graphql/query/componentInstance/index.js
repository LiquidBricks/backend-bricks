    ...
    data: {
      ...
      resolve: async (id, _args, { g }) => {
        const nodes = await loadInstanceStateNodes({
          g,
          instanceId: id,
          edgeLabel: domain.edge.has_data_state.componentInstance_data.constants.LABEL,
        });
    ...
    tasks: {
      ...
      resolve: async (id, _args, { g }) => {
        const nodes = await loadInstanceStateNodes({
          g,
          instanceId: id,
          edgeLabel: domain.edge.has_task_state.componentInstance_task.constants.LABEL,
        });
        return toConnection(nodes);
      },
    },
...
async function loadInstanceStateNodes({ g, instanceId, edgeLabel }) {
  ...
  const edgeIds = await g.V(instanceId).outE(edgeLabel).id();
  ...
  const nodes = await Promise.all(
    edgeIds.map(async (edgeId) => {
      ...
      const [nodeId] = await g.E(edgeId).outV().id();
