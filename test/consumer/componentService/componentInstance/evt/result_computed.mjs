...
import { componentImports } from '../../../../../consumer/componentService/componentInstance/cmd/create/loadData/componentImports.js'
...
test('result_computed publishes injected events across imports using import inject mappings', async () => {
  await withGraphContext(async ({ diagnostics, dataMapper, g }) => {
    const providerComponent = {
      hash: 'hash-import-inject-provider-events',
      name: 'ImportInjectProviderEvents',
      tasks: [
        { name: 'providerTask', fnc: 'fnProviderTask', codeRef: { file: 'pt.js', line: 1, column: 1 }, deps: [] },
      ],
      data: [
        { name: 'providerData', fnc: 'fnProviderData', codeRef: { file: 'pd.js', line: 2, column: 2 }, deps: [] },
      ],
    }
    const targetComponent = {
      hash: 'hash-import-inject-target-events',
      name: 'ImportInjectTargetEvents',
      tasks: [
        { name: 'targetTask', fnc: 'fnTargetTask', codeRef: { file: 'tt.js', line: 1, column: 1 }, deps: [] },
      ],
      data: [
        { name: 'targetData', fnc: 'fnTargetData', codeRef: { file: 'td.js', line: 2, column: 2 }, deps: [] },
      ],
    }
    const rootComponent = {
      hash: 'hash-import-inject-root-events',
      name: 'ImportInjectRootEvents',
      imports: [
        {
          name: 'target',
          hash: targetComponent.hash,
          inject: {
            'target.task.targetTask': ['provider.data.providerData', 'data.rootData'],
            'target.data.targetData': ['provider.task.providerTask'],
          },
        },
        { name: 'provider', hash: providerComponent.hash },
      ],
      tasks: [],
      data: [
        { name: 'rootData', fnc: 'fnRootData', codeRef: { file: 'rd.js', line: 1, column: 1 }, deps: [] },
      ],
    }
