...
const createEdgeId = () => `e-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 6)}`;

export default function SchemaCanvas({ onNodesUpdate = () => { }, exposeAddHelper, edgeLabels = [] }) {
  ...
  const createNode = useCallback((payload, position) => {
    ...
    return {
      ...
      data: { label, kind },
      style: {
        boxShadow: isEdge ? '0 0 18px rgba(34,197,94,0.15)' : '0 0 18px rgba(37,99,235,0.15)'
      },
    };
  }, []);
  ...
  const parseVertexName = useCallback((label) => {
    if (typeof label !== 'string') return null;
    const m = label.match(/^res\.vertex\.(.+)$/);
    return m ? m[1] : null;
  }, []);

  const parseEdgeInfo = useCallback((label) => {
    if (typeof label !== 'string') return null;
    const m = label.match(/^res\.edge\.([^\.]+)\.(.+?)_(.+)$/);
    if (!m) return null;
    return { edgeLabel: m[1], from: m[2], to: m[3] };
  }, []);

  const findVertexNodeByName = useCallback((name) => {
    if (!name) return null;
    return nodesRef.current.find(
      (node) => node?.data?.kind === 'vertex' && parseVertexName(node?.data?.label) === name
    );
  }, [parseVertexName]);
  ...
  const ensureVertexNode = useCallback((name) => {
    if (!name) return null;
    const existing = findVertexNodeByName(name);
    if (existing) return existing;
    const newNode = createNode({ kind: 'vertex', label: `res.vertex.${name}` }, defaultPositionForNewNode());
    nodesRef.current = [...nodesRef.current, newNode];
    setNodes([...nodesRef.current]);
    onNodesUpdate(nodesRef.current);
    return newNode;
  }, [createNode, defaultPositionForNewNode, findVertexNodeByName, onNodesUpdate]);

  const ensureGraphEdge = useCallback((sourceId, targetId) => {
  ...
  const autowireEdgesForVertex = useCallback((vertexName) => {
    if (!vertexName) return;
    let nodesChanged = false;
    let edgesChanged = false;

    (edgeLabels || []).forEach((label) => {
      const info = parseEdgeInfo(label);
      if (!info) return;
      if (info.from !== vertexName && info.to !== vertexName) return;

      const fromNode = findVertexNodeByName(info.from);
      const toNode = findVertexNodeByName(info.to);
      if (!fromNode || !toNode) return;

      let edgeNode = nodesRef.current.find(
        (node) => node?.data?.kind === 'edge' && node?.data?.label === label
      );
      if (!edgeNode) {
        edgeNode = createNode({ kind: 'edge', label }, defaultPositionForNewNode());
        nodesRef.current = [...nodesRef.current, edgeNode];
        nodesChanged = true;
      }

      const forward = ensureGraphEdge(fromNode.id, edgeNode.id);
      const backward = ensureGraphEdge(edgeNode.id, toNode.id);
      if (forward || backward) {
        edgesChanged = true;
      }
    });
  ...
  }, [createNode, defaultPositionForNewNode, edgeLabels, ensureGraphEdge, findVertexNodeByName, onNodesUpdate, parseEdgeInfo]);

  const wireEdgeNode = useCallback((edgeNode, edgeLabel) => {
    if (!edgeNode || !edgeLabel) return;
    const info = parseEdgeInfo(edgeLabel);
    if (!info) return;
    const fromNode = ensureVertexNode(info.from);
    const toNode = ensureVertexNode(info.to);
    if (!fromNode || !toNode) return;
  ...
  }, [ensureGraphEdge, ensureVertexNode, parseEdgeInfo]);
  ...
  const addNodeFromLabel = useCallback(
    (payload, position) => {
      ...
      if (payload?.kind === 'vertex') {
        const vertexName = parseVertexName(payload.label);
        autowireEdgesForVertex(vertexName);
      } else if (payload?.kind === 'edge') {
        wireEdgeNode(newNode, payload.label);
      }
    ...
    [autowireEdgesForVertex, createNode, onNodesUpdate, parseVertexName, saveState, wireEdgeNode]
  );
  ...
  const addByLabel = useCallback((payload) => {
    if (!payload || !payload.label || !payload.kind) return;
    if (payload.kind === 'vertex') {
      const pos = defaultPositionForNewNode();
      const newNode = createNode(payload, pos);
      nodesRef.current = [...nodesRef.current, newNode];
      setNodes(nodesRef.current);
      onNodesUpdate(nodesRef.current);
      const vertexName = parseVertexName(payload.label);
    ...
    if (payload.kind === 'edge') {
      const edgeNode = createNode({ kind: 'edge', label: payload.label }, defaultPositionForNewNode());
      nodesRef.current = [...nodesRef.current, edgeNode];
      setNodes([...nodesRef.current]);
      onNodesUpdate(nodesRef.current);
      wireEdgeNode(edgeNode, payload.label);
      maybeAutoSave('edge-node-added');
    }
  }, [autowireEdgesForVertex, createNode, defaultPositionForNewNode, maybeAutoSave, onNodesUpdate, wireEdgeNode]);

  useEffect(() => {
    if (typeof exposeAddHelper === 'function') {
      // Store function-as-state correctly
