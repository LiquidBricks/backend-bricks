...
export async function handler({ rootCtx: { diagnostics, g }, scope: { data: { instanceId, stateId, payload } } }) {
  diagnostics.require(typeof instanceId === 'string' && instanceId.length, Errors.PRECONDITION_REQUIRED, 'instanceId required for provide_data', { field: 'instanceId' })
  ...
  const instanceVertices = await g.V().has('label', 'componentInstance').has('instanceId', instanceId).id()
