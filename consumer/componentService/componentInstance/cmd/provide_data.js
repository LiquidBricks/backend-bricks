...
export async function handler({ rootCtx: { diagnostics, g }, scope: { data: { instanceId, stateId, payload } } }) {
  ...
  const stateMachineIds = await g
    .V(instanceVertexId)
    .out(domain.edge.has_stateMachine.componentInstance_stateMachine.constants.LABEL)
    .id()
  diagnostics.require(stateMachineIds?.length, Errors.PRECONDITION_INVALID, `stateMachine for componentInstance ${instanceId} not found`, { instanceId })
  const stateMachineId = stateMachineIds[0]
