...
export async function handler({ rootCtx: { g, natsContext }, scope: { handlerDiagnostics, instanceId, instanceVertexId, stateMachineId, providedNodeId, type } }) {
  const { dependentTaskNodeIds, dependentDataNodeIds } = await gatherDependentNodes({ g, providedNodeId, type })
  const instances = await collectInstanceChain({ g, startInstanceVertexId: instanceVertexId, startInstanceId: instanceId, startStateMachineId: stateMachineId })
...
async function publishStartCommands({ handlerDiagnostics, natsContext, instanceId, dataStateIds, taskStateIds }) {
  const publishList = [
