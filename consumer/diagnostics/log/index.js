...
function checkLevelThreshold({ scope: { level, [s.scope.abort]: abortCtl }, rootCtx: { diagnostics } }) {
...
// handler
function handler({ scope: { level, ts, attributes } }) {

  const time = (new Date(ts)).toLocaleTimeString()
  const out = `[${time}] ${level.toUpperCase()} ${attributes.msg}`
  // eslint-disable-next-line no-console
  console[level](out, attributes)
}
